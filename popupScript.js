class ZamaPopup {
    ...
    requestSubmitHandler(e) {
        ...
        if (inPlace === 'header') {
            if (requests['header'] && requests['header'].length) {
            requests['header'].unshift({ name, value, disabled: true, type: 'text' },{ name: 'x-registertype', value:2, disabled: true, type: 'text' });
            } else {
            requests['header'] = [{ name, value, disabled: true, type: 'text' },{ name: 'x-registertype', value:2, disabled: true, type: 'text' }];
            }
        } else {
            if (requests['query'] && requests['query'].length) {
            requests['query'].unshift({ name, value, disabled: true, type: 'text' },{ name: 'x-registertype', value:2, disabled: true, type: 'text' });
            } else {
            requests['query'] = [{ name, value, disabled: true, type: 'text' },{ name: 'x-registertype', value:2, disabled: true, type: 'text' }];
            }
        }
        }
